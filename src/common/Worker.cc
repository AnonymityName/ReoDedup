...
static char* readContainerTask(int con_id, string poolname, BaseFS* fs, int clen) {
    cout << "[Worker] readContainerTask conid:" << con_id << " poolname:" << poolname << " len:" << clen << endl;
    char* buf = (char*)calloc(clen, sizeof(char));
    if(!buf) {
        cerr << "[ERROR] Worker::readChunkTask malloc buffer error!" << endl;
        return nullptr;
...
//     redisReply* rReply = (redisReply*)redisCommand(sendCtx, "rpush %s 1", wkey.c_str());
//     cout << "[clientReorder] send reorder done reply to client, ip: " << _conf->_localIp << endl;
...
void Worker::clientReorder() {
    ...
    createConWorker(chunkInfos,chunks_fp_with_locality);
    end = system_clock::now();
    
    streambuf* coutBuf = cout.rdbuf();
    ofstream of("reorder.log", ios::app);
    streambuf* fileBuf = of.rdbuf();
    cout.rdbuf(fileBuf);

    cout <<  "[Worker clientReorder] reorder algorithm time is: " 
            << double(duration.count()) * milliseconds::period::num
            << " ms" << endl;
    duration = duration_cast<milliseconds>(end - start);
    cout <<  "[Worker clientReorder] reorder execute time is: " 
            << double(duration.count()) * milliseconds::period::num
            << " ms" << endl;
    ...
    int ipId;
    for( ipId = 0; ipId < _conf->_agentNum; ipId++) {
        if(_conf -> _localIp == _conf->_agentsIPs[ipId]) {
            break;
        }
    }
    ...
    redisReply* rReply;
    // cin.get();
    redisContext* sendCtx = RedisUtil::createContext(_conf->_agentsIPs[0]);
    rReply = (redisReply*)redisCommand(sendCtx, "rpush %s %b", wkey.c_str(),(void*)&ipId,sizeof(ipId));
    redisFree(sendCtx);
    assert(rReply!=NULL);
    assert(rReply->type == REDIS_REPLY_INTEGER);
    std::cout << "rReply->integer" << rReply->integer << endl;
    
    freeReplyObject(rReply);

    std::cout << "[clientReorder] send reorder done reply to client "<< _conf->_agentsIPs[0] <<", node " << ipId << endl;
...
void Worker::localRecognitionWorker(unordered_map<string,vector<ChunkInfo*> > &chunkInfos, vector<vector<string> > &chunks_fp_with_locality){
    ...
    vector<string> chunknames;  //key:hash_fp,value:ChunkInfo				
    //the chunknames of the chunks need to be reordered
    ...
    // for(auto it = chunknames.begin(); it != chunknames.end();) {
    //     if(chunkInfos[*it].size() > 1) {
    //         it++;
    //     } else {
    //         chunkInfos.erase(*it);
    //         it = chunknames.erase(it);
    //     }
    // }
    ...
    std::cout << "[Worker localrecognition] finish" << endl;
...
void Worker::createConCosWorker(unordered_map<string,vector<ChunkInfo*> > &chunkInfos, vector<vector<string> > &chunks_fp_with_locality){
    ...
    for(int i = 0; i < chunks_fp_with_locality.size(); i++) {
    	...
    	for(int j = 0; j < chunks_fp_with_locality[i].size(); j++) {
    	    ...
    	    recipe_name = chunkInfos[chunkname][0]->_filename+":"+poolname+":"+to_string(pktid);
 ...
 * 1 read the chunks
 * 2 push the chunks
 * 3 update the packet recipe parallelly
*/
void Worker::createConWorker(unordered_map<string,vector<ChunkInfo*> > &chunkInfos, vector<vector<string> > &chunks_fp_with_locality){
    ...
    string poolname = ""; 
    unordered_map<string, vector<WrappedFP*>*> recipeBuf;
    unordered_map<string, vector<tuple<int,int,int>>> recipe_update_info;   //key:recipe_name, value:<chunkid,conid,conoff>
    for(int i = 0; i < chunks_fp_with_locality.size(); i++) {
        std::cout << "chunks_fp_with_locality index: " << i << " ,size:" << chunks_fp_with_locality[i].size() << endl;
    	for(int j = 0; j < chunks_fp_with_locality[i].size(); j++) {
            ...
            // cout << "[Worker createConCosWorker] read packet recipe" << endl;
            ...
            int tmplen, dataLen;
            char* raw = read_container_buf.find(conName);
            // cout << "[Worker createConCosWorker] build and push chunk" << endl;
            ...
            //register the packet recipe need to update  
            vector<ChunkInfo*> info = chunkInfos[chunkname];
            for (int l = 0; l < info.size(); l++) {
            	string name = info[l]->_filename + ":" + info[l]->_poolname;
            	unordered_map<int, vector<int> > pkt_chunk_id = info[l]->pkt_chunk_id;
            	for (unordered_map<int, vector<int> >::iterator iter = pkt_chunk_id.begin(); iter != pkt_chunk_id.end(); iter++) {
                    string update_recipe_name = name + ":" + to_string(iter->first);
                    if(recipe_update_info.find(update_recipe_name) == recipe_update_info.end()){
                        int ipId = hash<string>()(update_recipe_name) % (_conf->_agentNum);
                        recipeIp[update_recipe_name] = ipId;
                    }
                    vector<int> chunkids = iter->second;
                    for (int j = 0; j < chunkids.size(); j++) {
                        recipe_update_info[update_recipe_name].push_back(make_tuple(chunkids[j],conid,conoff));
                    }                   
            	}
            }
            registerFp(chunkname,_conf->_localIp, 0, conid, conoff);
        ...
        if(poolname.size()) {
            DPDataChunk* lastChunk = new DPDataChunk();
            lastChunk->setLast(true);
            cout << "poolname:" << poolname << endl;
            _storeHandler->push(lastChunk, nullptr, nullptr, poolname);
            delete lastChunk;
        }
    }   
    read_container_buf.clear();
    adjustPacketRecipe(recipe_update_info, recipeIp);
    ...
    std::cout << "[createConCosWorker] finish create container" << endl;   
}

/**
 * 1. lock the packet recipe in the corresponding node
 * 2. read the packet recipe
 * 3. modify 
 * 4. store the packet recipe
 * 5. unlock
 * NOTE: recipe_update_info:key-recipename,value-<chunkid,conid,conoff>
*/
void Worker::adjustPacketRecipe(unordered_map<string,vector<tuple<int,int,int> > > &recipe_update_info,  unordered_map<string,int> recipe_ip){
    // std::cout << "[Worker adjustPacketRecipe] begin" << endl;
    for (unordered_map<string,vector<tuple<int,int,int> > >::iterator iter = recipe_update_info.begin(); 
                                                                iter != recipe_update_info.end(); iter++) {
        string recipe_name = iter -> first;
        vector<tuple<int,int,int> >& recipe_modify_info = iter -> second;
        int ipId = recipe_ip[recipe_name];
        vector<WrappedFP*>* recipe = new vector<WrappedFP*>();

        //lock
        lockPktRecipe(recipe_name, ipId);

        //read
        readPktRecipe(recipe_name, recipe, ipId);

        //modify
        while(!recipe_modify_info.empty()){
            tuple<int,int,int> info = recipe_modify_info.back();
            int chunkid = get<0>(info);
            int conid = get<1>(info);
            int conoff = get<2>(info);
            (*recipe)[chunkid]->_containerId = conid;
            (*recipe)[chunkid]->_offset = conoff;
            recipe_modify_info.pop_back();
        }
        
        //store
        writePktRecipe(recipe_name,recipe,ipId);

        //unlock
        unlockPktRecipe(recipe_name, ipId);
    ...
    // std::cout << "[Worker adjustPacketRecipe] finish" << endl;  
}

/**
 * try to lock packet recipe, if lock success, return, else sleep 1s and try again
 * NOTE: called before try to update the packet recipe in the radis
*/
void Worker::lockPktRecipe(string recipe_name, unsigned int ipId) {
    while(true) {
        redisReply* lock_reply;
        redisContext* lock_ctx = RedisUtil::createContext(_conf->_agentsIPs[ipId]);
        string lock_key = "lock_" + recipe_name;
        ...
        if(lock_reply->integer == 1) {
            ...
            redisFree(lock_ctx);
            // cout << "[Worker] lock packet recipe:" << recipe_name << " in node:" << ipId <<" success" << endl;
            break;
        } else {
            freeReplyObject(lock_reply);
            redisFree(lock_ctx);
...
/**
 * unlock packet recipe, if unlock success, return
 * NOTE: called after update the packet recipe in the radis
*/
void Worker::unlockPktRecipe(string recipe_name, unsigned int ipId){
    redisReply* unlock_reply;
    redisContext* unlock_ctx = RedisUtil::createContext(_conf->_agentsIPs[ipId]);
    string lock_key = "lock_" + recipe_name;
    ...
    // cout << "[Worker] unlock packet recipe:" << recipe_name << " in node:" << ipId <<" success" << endl;
}
...
void Worker::pushingWorker(BlockingQueue<DPDataChunk*>* hashQueue, string filename, string poolname, int pktid) {
    ...
    while(1) {
        ...
        // cout << "push pktid:" << pktid << " chunkid:" << chunkid << endl;
        int node_id = Router::chooseNode(chunk->getFp(), sizeof(fingerprint), _conf->_agentNum);
...
void Worker::clearReadBuf(){
    ...
    int io_cnt = readbuftmp->_io_cnt;
    readbuftmp->_io_cnt = 0;
    ...
    // del container in redis
    redisReply* getConainterInRedisReply = (redisReply*)redisCommand(_localCtx, "keys readbuf:container_*");
    assert(getConainterInRedisReply != NULL);
    assert(getConainterInRedisReply->type == REDIS_REPLY_ARRAY);
    			
    //the chunknames of the chunks need to be reordered
    for (int i = 0; i < getConainterInRedisReply->elements; i++) {
        const std::string key = getConainterInRedisReply->element[i]->str;
        redisReply* delContainerInRedisReply = (redisReply*)redisCommand(_localCtx, "DEL %s", key.c_str());
        assert(delContainerInRedisReply != NULL);
        assert(delContainerInRedisReply->type == REDIS_REPLY_INTEGER);
        assert(delContainerInRedisReply->integer == 1);
        freeReplyObject(delContainerInRedisReply);
    }
    freeReplyObject(getConainterInRedisReply);
}
