...
int Config::parseConf(string path) {
    ...
    while(getline(fs, line)) {
        ...
        else if (key == "reorder_mode") {
            stringstream ss(line.substr(pos+1, line.size()));
            string mode;
            ss >> mode;
            if (mode == "Freq") {
                _reorder_mode = ReorderMode::Freq;
            } else if (mode == "Cos") {
                _reorder_mode = ReorderMode::Cos;
            } else if (mode == "Chain") {
                _reorder_mode = ReorderMode::Chain;
            } else {
                std::cout << "unidentified reorder mode, support mode: Freq, Cos" << std::endl;
                exit(-1);
            }
        }
    }
