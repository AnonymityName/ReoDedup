...
void LocalRecognition::CalculateCosineSimilarity(){
    ...
    for(int i = 0; i < size; i++) {
        for(int j = i + 1; j < size; j++) {
            ...
            assert(tmp <= 1.1);
            assert(tmp >= 0);
			if(tmp == 1) mergeWorker(make_tuple(i,j,tmp));
            else if(tmp != 0) _locality_with_cos_sortedqueue.push(make_tuple(i,j,tmp));
...
void LocalRecognition::LocalRecognitionWorker() {
	...
	switch (_conf -> _reorder_mode)
	{
	case ReorderMode::Cos:
		LocalRecognitionWithCos();
		break;
	case ReorderMode::Freq:
		LocalRecognitionWithFreq();
		break;
	case ReorderMode::Chain:
		LocalRecognitionWithChain();
		break;
	default:
		cout << "undefined reorder mode!" << endl;
		break;
	}
...
/*
 * based on the chain address hash from file to chunk list
*/
void LocalRecognition::LocalRecognitionWithChain() {
	unordered_map<int, vector<int>> chainedFpTab; // file_idx -> fp list(in form of index)
	_chunks_fp_with_locality.resize(2, vector<string>(0)); // first degree: filesnum > totfile / 2; second degree: filesnum <= totfile / 2
	for(int i = 0; i < _chunks_fp.size(); i++) {
		string chunkName = _chunks_fp[i];
		int filesnum = count((_fileinfo_of_chunks[chunkName]).begin(),
							 (_fileinfo_of_chunks[chunkName]).end(),
							 '1');
		if(filesnum > _files_count / 2) {
			_chunks_fp_with_locality[0].push_back(chunkName);
		} else {
			for(int j = 0; j < _files_count; j++) {
				if(_fileinfo_of_chunks[chunkName][j] == '1') {
					chainedFpTab[j].push_back(i);
				}
			}
		}
	}
	cout << "[LocalRecognition::LocalRecognitionWithChain]: number of fp (filesnum > totfile / 2)" << _chunks_fp_with_locality[0].size() << endl;
	vector<bool> visited(_chunks_fp.size(), false);
	for(const auto & file : chainedFpTab) {
		for(const int fpIdx : file.second) {
			if(!visited[fpIdx]) {
				_chunks_fp_with_locality[1].push_back(_chunks_fp[fpIdx]);
				visited[fpIdx] = true;
			}
		}
	}
...
/*
 * based on the freqency, merge the chunkfp
*/
void LocalRecognition::LocalRecognitionWithFreq(){
	int size = _chunks_fp.size();
	for(int i = 0; i < size; i++) {
		string chunkname = _chunks_fp[i];
		int filesnum = count((_fileinfo_of_chunks[chunkname]).begin(),
							 (_fileinfo_of_chunks[chunkname]).end(),
							 '1');
		pair<int,string> files_of_a_chunk = make_pair(filesnum,chunkname);
		_locality_with_freq_sortedqueue.push(files_of_a_chunk);
	}
	_chunks_fp_with_locality.resize(1,vector<string>(0));
	while(!_locality_with_freq_sortedqueue.empty()){
		_chunks_fp_with_locality[0].push_back(_locality_with_freq_sortedqueue.top().second);
		_locality_with_freq_sortedqueue.pop();
	}
}
...
void LocalRecognition::LocalRecognitionWithCos(){
...
void LocalRecognition::mergeWorker(tuple<int, int, double> _cur){
	if(status[get<0>(_cur)] == 2 || status[get<1>(_cur)] == 2) return;    	
