...
// #define READ_CONTAINER_IN_REDIS
...
void StoreLayerWorker::popChunk(AGCommand* agCmd) {
    ...
    // cout << "register pktid:" << pktid << " chunkid:" << chunkid << endl;
...
void StoreLayerWorker::popBatchChunk(AGCommand* agCmd) {
    ...
    for(int i=0;i<chunk_cnt;i++) {      // | chunkid | fingerprint | datalen | data |
        ...
        int con_id, con_off, dup;
        string chunkname = to_string(Router::hash(chunk->getFp(), sizeof(fingerprint)));
        // cout << "pktid:" << pktid << " chunkid:" << chunkid << endl;
        if(!res.empty()) {          // duplicate chunk
            con_id=res[1];
            con_off=res[2];
            dup = 1;
...
void StoreLayerWorker::readPacket(AGCommand* agCmd) {
    ...
    for(int i = 0; i < recipe->size(); i++) {
        ...
        if (readbuf->find(conName) == NULL) {
            ...
            // check in redis
            unsigned int ipId = con_id % _conf->_agentNum;
            redisContext* checkRedisContainerCtx = RedisUtil::createContext(_conf->_agentsIPs[ipId]);
            const std::string checkRedisContainerKey = "readbuf:" + conName;
            redisReply* checkRedisContainerReply = (redisReply*)redisCommand(checkRedisContainerCtx, "GET %s", checkRedisContainerKey.c_str());
            assert(checkRedisContainerReply != NULL);
            
            char* buf;
            int len = _conf->_containerSize*(sizeof(int) + _conf->_chunkMaxSize);
            if (checkRedisContainerReply->type == REDIS_REPLY_STRING) {    // if find in redis
                std::cout << "[readPacket] redis hit" << std::endl;
                buf = new char [len];
                char* buf_temp = checkRedisContainerReply->str;
                memcpy(buf, buf_temp, len);
            } else {                                                       // not found in redis, read in ceph
                readbuf->_io_cnt++;
                buf = _storeHandler->readContainerTask(con_id, poolname, _conf->_containerSize*(sizeof(int) + _conf->_chunkMaxSize));
#ifdef READ_CONTAINER_IN_REDIS
                std::cout << "[readPacket] redis not hit, set " << checkRedisContainerKey << std::endl;
                redisReply* setRedisContainerReply = (redisReply*)redisCommand(checkRedisContainerCtx, "SET %s %b", checkRedisContainerKey.c_str(), (void*)buf, len);
                freeReplyObject(setRedisContainerReply);
#endif
            }
            freeReplyObject(checkRedisContainerReply);
            redisFree(checkRedisContainerCtx);
